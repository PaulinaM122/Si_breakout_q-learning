...
while training_agent:
    ...
    while playing_game:
        ...
        # perform action
        left_side = -width/2 + HALF_PADDLE
        right_side = width/2 - HALF_PADDLE
        if next_action == Dir.LEFT.value and paddle.xcor() > left_side:
            paddle.move_left()
        elif next_action == Dir.RIGHT.value and paddle.xcor() < right_side:
            paddle.move_right()
